 ...
 */
package net.opticlient.serverfinder;
...
import net.opticlient.mixinterface.IMultiplayerScreen;
...
public class ServerFinderScreen extends Screen
{
			...
			ButtonWidget
				.builder(Text.literal("Tutorial"),
					b -> Util.getOperatingSystem().open(
						"https://www.OptiClient.net/serverfinder-tutorial/"))
	...
	private void findServers()
	{
		try
		{
			...
			ArrayList<OptiServerPinger> pingers = new ArrayList<>();
			...
			for(int change : changes)
				for(int i2 = 0; i2 <= 255; i2++)
				{
					...
					OptiServerPinger pinger = new OptiServerPinger();
					pinger.ping(ip);
					pingers.add(pinger);
	...
	private void updatePingers(ArrayList<OptiServerPinger> pingers)
	{
		for(int i = 0; i < pingers.size(); i++)
		{
			OptiServerPinger pinger = pingers.get(i);
