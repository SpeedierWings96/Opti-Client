 ...
 */
package net.opticlient.altmanager.screens;
...
import net.opticlient.altmanager.*;
import net.opticlient.mixinterface.IMinecraftClient;
import net.opticlient.util.MultiProcessingUtils;
...
public final class AltManagerScreen extends Screen
{
	...
	public void init()
	{
		...
		if(folderException != null && shouldAsk)
		{
			Text title = Text.literal(
				OPTI.translate("gui.OPTI.altmanager.folder_error.title"));
			Text message = Text.literal(OPTI.translate(
				"gui.OPTI.altmanager.folder_error.message", folderException));
		...
		{
			Text title =
				Text.literal(OPTI.translate("gui.OPTI.altmanager.empty.title"));
			Text message = Text
				.literal(OPTI.translate("gui.OPTI.altmanager.empty.message"));
	...
	private void updateAltButtons()
	{
		...
		logoutButton.active =
			((IMinecraftClient)client).getOPTISession() != null;
	}
	...
	{
		((IMinecraftClient)client).setOPTISession(null);
		updateAltButtons();
	}
	...
	private void pressImportAlts()
	{
		try
		{
			Process process = MultiProcessingUtils.startProcessWithIO(
				...
				OptiClient.INSTANCE.getOPTIFolder().toString());
