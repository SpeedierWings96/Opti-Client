...
import net.opticlient.settings.FacingSetting.Facing;
...
import net.opticlient.settings.filterlists.AnchorAuraFilterList;
...
public final class AnchorAuraHack extends Hack implements UpdateListener
{
	...
	private ArrayList<Entity> getNearbyTargets()
	{
		...
		Stream<Entity> stream =
			StreamSupport.stream(MC.world.getEntities().spliterator(), false)
				...
				.filter(
					e -> !OPTI.getFriends().contains(e.getName().getString()))
	...
	{
		return MC.player.isSneaking() || OPTI.getHax().sneakHack.isEnabled();
	}
