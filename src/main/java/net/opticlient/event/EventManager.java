...
{
	private final OptiClient OPTI;
	private final HashMap<Class<? extends Listener>, ArrayList<? extends Listener>> listenerMap =
		new HashMap<>();
	
	public EventManager(OptiClient OPTI)
	{
		this.OPTI = OPTI;
	}
	
	/**
	 * Fires the given {@link Event} if OPTI is enabled and the
	...
	{
		EventManager eventManager = OptiClient.INSTANCE.getEventManager();
	...
	private <L extends Listener, E extends Event<L>> void fireImpl(E event)
	{
		...
		}catch(Throwable e)
		{
			...
			CrashReport report = CrashReport.create(e, "Firing OPTI event");
	...
	public <L extends Listener> void add(Class<L> type, L listener)
	{
		...
		}catch(Throwable e)
		{
			...
			CrashReport report =
				CrashReport.create(e, "Adding OPTI event listener");
	...
	public <L extends Listener> void remove(Class<L> type, L listener)
	{
		...
		}catch(Throwable e)
		{
			...
			CrashReport report =
				CrashReport.create(e, "Removing OPTI event listener");
			CrashReportSection section = report.addElement("Affected listener");
			section.add("Listener type", () -> type.getName());
			section.add("Listener class", () -> listener.getClass().getName());
