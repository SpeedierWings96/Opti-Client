...
import net.opticlient.util.MathUtils;
...
public final class BindsCmd extends Command
{
	public BindsCmd()
	{
		super("binds", "Allows you to manage keybinds through the chat.",
			...
			"Profiles are saved in '.minecraft/opti/keybinds'.");
	}
	...
	private void add(String[] args) throws CmdException
	{
		...
		OPTI.getKeybinds().add(key, commands);
	...
	private void remove(String[] args) throws CmdException
	{
		...
		String commands = OPTI.getKeybinds().getCommands(key);
		if(commands == null)
			throw new CmdError("Nothing to remove.");
		
		OPTI.getKeybinds().remove(key);
	...
	private void list(String[] args) throws CmdException
	{
		...
		List<Keybind> binds = OPTI.getKeybinds().getAllKeybinds();
	...
	{
		OPTI.getKeybinds().removeAll();
		ChatUtils.message("All keybinds removed.");
	}
	
	private void reset()
	{
		OPTI.getKeybinds().setKeybinds(KeybindList.DEFAULT_KEYBINDS);
	...
	private void loadProfile(String[] args) throws CmdException
	{
		...
		try
		{
			OPTI.getKeybinds().loadProfile(name);
	...
	private void saveProfile(String[] args) throws CmdException
	{
		...
		try
		{
			OPTI.getKeybinds().saveProfile(name);
	...
	private void listProfiles(String[] args) throws CmdException
	{
		...
		ArrayList<Path> files = OPTI.getKeybinds().listProfiles();
		int page = parsePage(args);
