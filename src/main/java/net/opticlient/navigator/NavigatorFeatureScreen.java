 ...
 */
package net.opticlient.navigator;
...
import net.opticlient.keybinds.PossibleKeybind;
...
public final class NavigatorFeatureScreen extends NavigatorScreen
{
			...
			primaryButton =
				ButtonWidget.builder(Text.literal(primaryAction), b -> {
					TooManyHaxHack tooManyHax =
						OptiClient.INSTANCE.getHax().tooManyHaxHack;
					...
					OptiClient.INSTANCE.getNavigator()
		...
		if(!possibleKeybinds.isEmpty())
		{
			...
			ButtonData addKeybindButton =
					...
					area.y + getStringHeight(text) - 7, 12, 8, "+", 0x00ff00)
				{
					...
					{
						// add keybind
						OptiClient.MC.setScreen(new NavigatorNewKeybindScreen(
			...
			for(Keybind keybind : OptiClient.INSTANCE.getKeybinds()
				.getAllKeybinds())
			{
	...
	{
		// popups
		if(OptiClient.INSTANCE.getGui().handleNavigatorPopupClick(x, y, button))
			return;
		...
		if(activeButton != null)
		{
			...
			activeButton.press();
			OptiClient.INSTANCE.getNavigator().addPreference(feature.getName());
			return;
		}
		
		// component settings
		OptiClient.INSTANCE.getGui().handleNavigatorMouseClick(
	...
	{
		OptiClient.INSTANCE.getGui().handleMouseRelease(x, y, button);
	}
		...
		float partialTicks)
	{
		...
		ClickGui gui = OptiClient.INSTANCE.getGui();
		...
		for(ButtonData buttonData : buttonDatas)
		{
			...
			context.drawCenteredTextWithShadow(client.textRenderer,
				...
				y1 + (buttonData.height - 10) / 2 + 1, buttonData.isLocked()
					? OptiColors.VERY_LIGHT_GRAY : buttonData.textColor);
	...
	{
		window.close();
		OptiClient.INSTANCE.getGui().handleMouseClick(Integer.MIN_VALUE,
			Integer.MIN_VALUE, 0);
	}
