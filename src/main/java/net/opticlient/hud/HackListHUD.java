...
import net.opticlient.other_features.HackListOtf;
import net.opticlient.other_features.HackListOtf.Mode;
import net.opticlient.other_features.HackListOtf.Position;

public final class HackListHUD implements UpdateListener
{
	private final ArrayList<HackListEntry> activeHax = new ArrayList<>();
	private final HackListOtf otf = OptiClient.INSTANCE.getOtfs().hackListOtf;
	private int posY;
	private int textColor;
	
	public HackListHUD()
	{
		OptiClient.INSTANCE.getEventManager().add(UpdateListener.class, this);
	}
	...
	public void render(DrawContext context, float partialTicks)
	{
		...
		if(otf.getPosition() == Position.LEFT
			&& OptiClient.INSTANCE.getOtfs().OptiLogoOtf.isVisible())
			posY = 22;
		else
			posY = 2;
