...
public enum XRayHackTest
{
	...
	public static void testXRayHack()
	{
		...
		runOPTICommand("setcheckbox X-Ray only_show_exposed off");
		runOPTICommand("setslider X-Ray opacity 0");
		runOPTICommand("t X-Ray on");
		takeScreenshot("xray_default", Duration.ofMillis(300));
		runOPTICommand("t X-Ray off");
		clearChat();
		
		// Exposed only
		runOPTICommand("setcheckbox X-Ray only_show_exposed on");
		runOPTICommand("setslider X-Ray opacity 0");
		runOPTICommand("t X-Ray on");
		...
		// Opacity mode
		runOPTICommand("setcheckbox X-Ray only_show_exposed off");
		runOPTICommand("setslider X-Ray opacity 0.5");
		runOPTICommand("t X-Ray on");
