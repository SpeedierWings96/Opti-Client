...
public enum OptiClientTestHelper
{
	;
	...
	/**
	 * Runs the given OPTI command.
	 *
	 * <p>
	 * Do not put a . at the start of the command.
	 */
	public static void runOPTICommand(String command)
	{
		System.out.println("Running command: ." + command);
		submitAndWait(mc -> {
			OptiClient.INSTANCE.getCmdProcessor().process(command);
		});
	}
