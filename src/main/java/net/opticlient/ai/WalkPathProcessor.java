...
public class WalkPathProcessor extends PathProcessor
{
	...
	public void process()
	{
		// get positions
		BlockPos pos;
		if(OptiClient.MC.player.isOnGround())
			pos = BlockPos.ofFloored(OptiClient.MC.player.getX(),
				OptiClient.MC.player.getY() + 0.5, OptiClient.MC.player.getZ());
		else
			pos = BlockPos.ofFloored(OptiClient.MC.player.getPos());
		...
		lockControls();
		OptiClient.MC.player.getAbilities().flying = false;
		...
		if(OPTI.getHax().jesusHack.isEnabled())
		{
			// wait for Jesus to swim up
			if(OptiClient.MC.player.getY() < nextPos.getY()
				&& (OptiClient.MC.player.isTouchingWater()
					|| OptiClient.MC.player.isInLava()))
				return;
			
			// manually swim down if using Jesus
			if(OptiClient.MC.player.getY() - nextPos.getY() > 0.5
				&& (OptiClient.MC.player.isTouchingWater()
					|| OptiClient.MC.player.isInLava()
					|| OPTI.getHax().jesusHack.isOverLiquid()))
		...
		if(pos.getX() != nextPos.getX() || pos.getZ() != nextPos.getZ())
		{
			...
			if(pos.getY() < nextPos.getY())
			{
				...
				{
					OPTI.getRotationFaker().faceVectorClientIgnorePitch(
