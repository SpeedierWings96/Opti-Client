...
public enum RenderUtils
{
	...
	{
		Camera camera = OptiClient.MC.getBlockEntityRenderDispatcher().camera;
		if(camera == null)
			return Vec3d.ZERO;
	...
	{
		return OptiClient.MC.getBufferBuilders().getEntityVertexConsumers();
	}
		...
		int color, boolean depthTest)
	{
		...
		RenderLayer layer = OptiRenderLayers.getLines(depthTest);
	...
	{
		VertexConsumerProvider.Immediate vcp = getVCP();
		RenderLayer layer = OptiRenderLayers.getLineStrip(depthTest);
		VertexConsumer buffer = vcp.getBuffer(layer);
	...
	{
		VertexConsumerProvider.Immediate vcp = getVCP();
		RenderLayer layer = OptiRenderLayers.getQuads(depthTest);
		VertexConsumer buffer = vcp.getBuffer(layer);
		...
		int y, boolean large)
	{
		...
		if(stack.isEmpty())
		{
			...
			context.drawText(tr, "?", 3, 2, OptiColors.VERY_LIGHT_GRAY, true);
	...
	public static void fill2D(DrawContext context, float x1, float y1, float x2,
		float y2, int color)
	{
		int scale = OptiClient.MC.getWindow().getScaleFactor();
	...
	public static void drawBoxShadow2D(DrawContext context, int x1, int y1,
		int x2, int y2)
	{
		float[] acColor = OptiClient.INSTANCE.getGui().getAcColor();
