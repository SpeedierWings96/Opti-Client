...
public final class NcrModRequiredScreen extends Screen
{
	...
	{
		super(Text.literal(ChatUtils.opti_PREFIX + OptiClient.INSTANCE
			.translate("gui.OPTI.nochatreports.ncr_mod_server.title")));
		this.prevScreen = prevScreen;
		
		reason = Text.literal(OptiClient.INSTANCE
			.translate("gui.OPTI.nochatreports.ncr_mod_server.message"));
		
		OtfList otfs = OptiClient.INSTANCE.getOtfs();
	...
	{
		return OptiClient.INSTANCE.translate("options." + (on ? "on" : "off"))
			.toUpperCase();
	}
	...
	{
		OptiClient.INSTANCE.getOtfs().vanillaSpoofOtf.doPrimaryAction();
