 ...
 */
package net.opticlient.nochatreports;
...
import net.opticlient.other_features.NoChatReportsOtf;
import net.opticlient.util.ChatUtils;
import net.opticlient.util.LastServerRememberer;
...
public final class ForcedChatReportsScreen extends Screen
{
	...
	{
		super(Text.literal(ChatUtils.opti_PREFIX)
			.append(Text.literal(OptiClient.INSTANCE
				.translate("gui.OPTI.nochatreports.unsafe_server.title"))));
		this.prevScreen = prevScreen;
		
		reason = Text.literal(OptiClient.INSTANCE
			.translate("gui.OPTI.nochatreports.unsafe_server.message"));
		
		NoChatReportsOtf ncr = OptiClient.INSTANCE.getOtfs().noChatReportsOtf;
		sigButtonMsg = () -> OptiClient.INSTANCE
			.translate("button.OPTI.nochatreports.signatures_status")
			+ blockedOrAllowed(ncr.isEnabled());
	}
	
	private String blockedOrAllowed(boolean blocked)
	{
		return OptiClient.INSTANCE.translate(
			"gui.OPTI.generic.allcaps_" + (blocked ? "blocked" : "allowed"));
	}
	...
	{
		OptiClient.INSTANCE.getOtfs().noChatReportsOtf.doPrimaryAction();
